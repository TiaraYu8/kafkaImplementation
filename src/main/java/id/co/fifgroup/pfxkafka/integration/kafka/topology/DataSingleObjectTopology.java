...
@Component
public class DataSingleObjectTopology {
    ...
    public void topologyProcess(StreamsBuilder streamsBuilder) {
        KStream<String, KafkaDataStreamDTO> dataStream = streamsBuilder
                ...
                .filter((s, kafkaDataStreamDTO) -> DataFieldTypeEnum.SINGLE.getType().equals(kafkaDataStreamDTO.getFieldType()));
        ...
        KTable<String, List<KafkaDataStreamDTO>> dataTable = dataStream.map((key, data) ->
                ...
                .aggregate(ArrayList::new,
                        (key, value, aggregate) -> {
                            aggregate.add(value);
                            return aggregate;
                        }, Materialized.<String, List<KafkaDataStreamDTO>, KeyValueStore<Bytes, byte[]>>as(KTableLabelConstants.KTABLE_LABEL_SINGLE_OBJECT_DATA)
        ...
        dataTable.toStream().foreach((dataUUID, kafkaDataStreamDTOS) -> {
            ...
            JsonNode dataPayloadFromContext = DataPayloadContextHolder.getInstance().getDataPayload(dataUUID, DataFieldTypeEnum.SINGLE.getType());
            ...
            notifDTO.setDataType(DataFieldTypeEnum.SINGLE.getType());
